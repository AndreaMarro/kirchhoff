...
def _scrivi_vincolo(ir: IR, prima: DerivationState) -> tuple[AnalyticalStep, DerivationState]:
    candidati = supernodi_semplici(ir)
    if not candidati:
        raise ValueError(
            "nessun vincolo di tensione da supernodo semplice in questo slice")
    return scrivi_vincolo_tensione(ir, prima, candidati[0].source_id)
...
def applica_passo(
    ...
    if kind == "write_voltage_constraint":
        return _scrivi_vincolo(ir, prima)
    raise ValueError(
