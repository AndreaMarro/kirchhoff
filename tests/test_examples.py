"""5 circuiti curati devono risolvere e verificare in <60s."""
...
def test_examples_curati():
    for name in ("series", "parallel", "ladder", "bridge", "nodal"):
        ...
        # SVG solo per maglia singola
        if name in ("series", "ladder"):
            assert esito.svg is not None and esito.svg.startswith("<svg")
        else:
            # non-maglia: solve ok, svg None è il comportamento corretto
            pass
