...
from kirchhoff.pipeline.resolve import Solved, VERIFICHE, resolve
...
def test_il_partitore_passa_tutto_lo_spine():
    ...
    assert esito.verifiche == (
        "Claim elettrico: VERIFIED", "Sessione backend: CLOSED")
...
def test_il_fasore_rifiuta_sul_percorso_certificato():
...
def test_sinusoidale_reattivo_senza_omega_si_rifiuta():
    from kirchhoff.domain.ir import Request
    ir = IR("1.0.0", "ac_sinusoidal", "generated", ("0", "A"),
            (Component.of("R1", "resistor", ("A", "0"), F(10), "R_1"),
             Component.of("C1", "capacitor", ("A", "0"), F(1, 1000), "C_1")),
            (Request("q1", "voltage", "R1"),), F(0))
    esito = resolve(ir)
    assert isinstance(esito, Refusal)
    assert esito.cause == "unsolvable"
...
def test_eccezione_inattesa_del_solver_nomina_lo_stadio(monkeypatch):
    ...
    assert isinstance(esito, Failure)
    assert esito.dove == "orchestrate"
    assert "heap" in esito.messaggio
...
def test_verify_che_esplode_nomina_lo_stadio(monkeypatch):
    import kirchhoff.domain.truthfulness as gate
    monkeypatch.setattr(gate, "verify", lambda ir, sol: (_ for _ in ()).throw(RuntimeError("residuo rotto")))
    esito = resolve(leggi(PARTITORE))
    assert isinstance(esito, Failure)
    assert esito.dove == "orchestrate"
