...
| Identificatori | ULID con prefisso per tipo (`ir_`, `sol_`, `var_`, `evt_`; dalla v2.1 anche `lay_` per il `LayoutIR` e `patch_` per il `LayoutPatch`, che senza identità non sono citabili da evidenza, replay ed eval; dalla Proof Demo 0.1 anche `sess_` per l'occurrence della `ProofSession` pubblicata). Mai interi auto-incrementali su entità esposte. Nota di emendamento (H2.5, decisione `docs/decisions/2026-09-03-proofsession-serialization-readiness.md`, D-H2.5-6): `sol_` resta il `Published` scritto da `solve` e non nomina la sessione; riusarlo solo perché esiste sarebbe abuso di genere. Writer unico di `sess_`: il compositore applicativo (`pipeline/proof_session.compose_proof_session`), con istante ed entropia iniettati (AD-17). |
